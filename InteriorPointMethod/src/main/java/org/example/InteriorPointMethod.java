...
import org.ejml.simple.SimpleMatrix; //just a library for operations with matrices, it only has a computational meaning

public class InteriorPointMethod {
    public static SimpleMatrix interiorPointMethod(SimpleMatrix A, SimpleMatrix b, SimpleMatrix c, SimpleMatrix x0, double alpha, double epsilon) {
        ...
        while (!solved) {
            ...
            try {
                ...
                SimpleMatrix P = SimpleMatrix.identity(x.numRows()).minus(A_temp_t.mult(AAtInv).mult(A_temp));
    ...
    public static void tester() {
        ...
        double[][] x0DataList = {
                {3, 3}, {1, 2, 3}, {0.5, 1, 1.5, 0.5}, {2, 0.5, 2, 0.5, 4.5}, {1, 1, 1}, {1, 1}
        };
        double[] epsilonList = {
        ...
        for (int i = 0; i < aDataList.length; i++){
            ...
            SimpleMatrix b = new SimpleMatrix(bDataList[i].length, 1, true, bDataList[i]);
            SimpleMatrix c = new SimpleMatrix(cDataList[i].length, 1, true, cDataList[i]);
            SimpleMatrix x0 = new SimpleMatrix(x0DataList[i].length, 1, true, x0DataList[i]);

            System.out.println("Test №" + (i+1) + ":");

            SimpleMatrix result1 = interiorPointMethod(A, b, c, x0, alpha1, epsilonList[i]);
            ...
            if (result1 == null) {
                ...
                System.out.print("α = 0.5: ");
                printVector(result1);
                System.out.print("α = 0.9: ");
                printVector(result2);
                System.out.print("by simplex method: ");
                Object[] result = simplexMethod(cDataList[i], aDataList[i], bDataList[i], precision, epsilonList[i]);
